...
Mode::AltHoldModeState Mode::get_alt_hold_state(float target_climb_rate_cms)
{
    ...
    if (!motors->armed()) {//����������
    ...
    } else if (takeoff.running() || takeoff.triggered(target_climb_rate_cms)) {//������������ɻ򴥷����
        ...
        return AltHold_Takeoff;//���÷�����״̬Ϊ���

    }
    else if (!copter.ap.auto_armed || copter.ap.land_complete) {  //������������δ�ڿ��в���δ�������
        // the aircraft is armed and landed
        if (target_climb_rate_cms < 0.0f && !copter.ap.using_interlock) {//Ŀ�������ʹ���
            // the aircraft should move to a ground idle state
            motors->set_desired_spool_state(AP_Motors::DesiredSpoolState::GROUND_IDLE);//������浡��״̬

        } else {
            // the aircraft should prepare for imminent take off
            motors->set_desired_spool_state(AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED);//����׼�����״̬
        }
